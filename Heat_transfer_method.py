...
def bottom(T, y, x, h, amb_T, H, k, q):
...
def interior(T, y, x, h, k, q):
    c = np.square(h) * q / k
    updated_T = 0.25 * (c - T[y - 1, x] - T[y + 1, x] - T[y, x - 1] - T[y, x + 1])

    return updated_T

def Jacobi_Solve(x_range, y_range, h = 0.00001, initial_guess=0, natural=True):
    # definitions

    initial_x_dim = (x_range / h) + 2
    initial_y_dim = (y_range / h) + 2
